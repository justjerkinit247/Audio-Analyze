...
from pipeline_batch import analyze_folder


def test_pipeline_batch_analyze_folder(tmp_path):
    ...
    result = analyze_folder(input_dir=input_dir, output_dir=output_dir)

    assert result['files_processed'] == 2
    assert (output_dir / 'summary.csv').exists()
    assert (output_dir / 'prompt_profiles.txt').exists()
    assert (output_dir / 'video_cues.txt').exists()
