...
def build_scene_control_status(plan_json, preflight_report, output_json):
    ...
    status = {
        "status": "PASSED" if not mapping.get("problems") and preflight.get("status") == "PASSED" else "NEEDS_ATTENTION",
        ...
        "scenes": [],
        "problems": list(mapping.get("problems", [])) + list(preflight.get("problems", [])),
    }
...
def main():
    parser = argparse.ArgumentParser(description="One-shot LTX scene-control prep: apply seed mapping, preview, preflight, and write status.")
