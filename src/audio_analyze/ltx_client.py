...
class LTXClient:
    ...
    def image_to_video(self, image_uri, prompt, output_path, model="ltx-2-3-pro", duration=8, resolution="1080x1920", fps=24, guidance_scale=9.0):
        payload = {
            "image_uri": self.ensure_uri(image_uri),
            "prompt": prompt,
            "model": model,
            "duration": duration,
    ...
    def audio_to_video(self, audio_uri, prompt, output_path, image_uri=None, model="ltx-2-3-pro", resolution="1080x1920", guidance_scale=9.0):
        payload = {
            "audio_uri": self.ensure_uri(audio_uri),
            "prompt": prompt,
            "model": model,
            "resolution": resolution,
            "guidance_scale": guidance_scale,
        ...
        if image_uri:
            payload["image_uri"] = self.ensure_uri(image_uri)
