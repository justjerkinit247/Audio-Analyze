...
def init_memory_bank(state_root: Path) -> dict:
    ...
    defaults = {
        "movement_skills.json": {
            "skills": {
                "downbeat_locked_motion": {"weight": 1.0, "description": "Visible performance accents land on strong beats."},
                "simple_repeating_phrase": {"weight": 1.0, "description": "One clear repeated movement phrase per scene."},
            }
        },
        "camera_skills.json": {
            "skills": {
                "single_controlled_camera_move": {"weight": 1.0, "description": "One stable camera move rather than competing camera instructions."},
                "camera_follows_motion": {"weight": 1.0, "description": "Camera motion follows visible performance rhythm."},
            }
        },
        "prompt_rules.json": {
            "rules": {
                "compress_when_obedience_low": True,
                "remove_conflicting_directives": True,
                "preserve_winning_fragments": True,
                "prioritize_seed_fidelity_when_drift_detected": True,
            }
        },
    }
...
def update_skill_weight(file_path: Path, skill_name: str, delta: float) -> None:
    ...
    skill = data.setdefault("skills", {}).setdefault(skill_name, {"weight": 1.0, "description": "Auto-created skill."})
...
def update_memory_from_active_state(state_root: Path) -> dict:
    ...
    for scene in feedback.get("scene_feedback", []):
        ...
        if "winning_pattern_candidate" in issues:
            append_jsonl(root / "winning_patterns.jsonl", {"type": "scene", "scene_id": scene_id, "scores": scores, "note": "Scene marked as winning pattern candidate."})
        for issue in issues:
            if issue not in {"winning_pattern_candidate"}:
...
def main():
    ...
    p_summary.add_argument("--state-root", default="outputs/ltx_video_run/_state")
