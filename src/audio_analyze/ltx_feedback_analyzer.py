...
def score_or_default(value, default: float) -> float:
    """Backward-compatible helper; feedback analysis does not invent defaults."""
...
def analyze_feature(row: dict) -> dict:
    ...
    scored_metrics = [
        metric for metric, source in score_evidence.items()
        if source == SCORE_SOURCE_HUMAN
    ]

    return {
        "scene_id": row.get("clip_index"),
        "scores": {
            metric: _rounded_score(value) for metric, value in score_values.items()
        },
        "score_evidence": score_evidence,
        "scored_metrics": scored_metrics,
        "unscored_metrics": [
            metric for metric in score_values if metric not in scored_metrics
        ],
...
def build_feedback_packet(state_root: Path) -> dict:
    ...
    summary = {
        ...
        "scored_scene_count": sum(
            1 for scene in scene_feedback if scene.get("scored_metrics")
        ),
        "fully_unscored_scene_count": sum(
            1 for scene in scene_feedback if not scene.get("scored_metrics")
        ),
    }
