...
def classify_audio_source(audio_path: Path) -> dict:
    ...
    return {"path": str(audio_path), "suffix": suffix, "quality_class": quality_class, "recommendation": recommendation}
...
def analyze_beat_grid(audio_path: Path, hop_length: int = 512) -> dict:
    ...
    stability = max(0.0, min(1.0, 1.0 - (interval_std or 0.0) / interval_mean)) if interval_mean and interval_mean > 0 else 0.0
...
def score_scene_boundaries(plan: dict, beat_report: dict) -> dict:
    ...
    for item in plan.get("results", []):
        ...
        scenes.append({"clip_index": item.get("clip_index"), "start": start, "end": end, "start_nearest_beat_distance": round(start_dist, 4) if start_dist is not None else None, "end_nearest_beat_distance": round(end_dist, 4) if end_dist is not None else None, "boundary_confidence": round((start_score + end_score) / 2.0, 4)})
